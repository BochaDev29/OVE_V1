...
function grillaAPixeles(
    ...
    const totalContentWidth = pos.colSpan * CELDA_CONFIG.ancho;
    ...
    if (mainHeader) {
        ...
        if (fedCircuits.length > 1) {
            ...
            fedCircuits.forEach((circuit: any, index: number) => {
                ...
                const piaType = (isTrifasico && circuit.cable?.conductors >= 4 ? SYMBOL_MAP.PIA_3P : SYMBOL_MAP.PIA_1P);
                ...
                if (panel.grounding?.hasPAT) {
                    pipes.push({
                        id: generateSymbolId(`pat-line-${circuit.id}`),
                        type: 'line',
                ...
                const ctType = isTrifasico && circuit.cable?.conductors >= 4 ? SYMBOL_MAP.CT_380 : SYMBOL_MAP.CT_220;
