...
export function generateUnifilarDiagram(
    ...
    // Detectar si es trifásico
    const isTrifasico = config.acometida?.phases === 3;

    console.log('🔧 Generando diagrama unifilar:', {
        isTrifasico,
    ...
    console.log(`✅ Diagrama generado: ${symbols.length} símbolos`);
...
/**
 * Genera símbolos de un tablero (rectángulo + protecciones + circuitos)
 */
function generatePanel(
    ...
    panelCircuits.forEach((circuit, index) => {
        // PIA del circuito
        const piaLabel = generatePIALabel(circuit, isTrifasico);
        const piaType = isTrifasico && circuit.cable?.conductors === 4 ? 'tm_4p' : 'tm_2p';
...
/**
 * Genera etiqueta técnica para un cable
 */
