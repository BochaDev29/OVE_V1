...
function generateAcometida(
    ...
    const mainBreakerType = isTrifasico ? SYMBOL_MAP.PIA_3P : SYMBOL_MAP.PIA_1P;

    symbols.push({
        id: generateSymbolId('main-breaker'),
        type: mainBreakerType as any,
        x,
        y: currentY,
        rotation: 0,
        layer: 'layer-0',
...
function generatePanel(
    ...
    if (mainHeader) {
        const headerType = mainHeader.type === 'ID'
        ...
        // 2.1 BORNERA PAT (a la izquierda, como en el ejemplo FIG 171)
        const patBusbarX = x - 100; // Columna auxiliar a la izquierda
        if (panel.grounding?.hasPAT) {
            symbols.push({
                id: generateSymbolId('ground-busbar'),
                type: 'text' as any, // temporal hasta símbolo específico
                x: patBusbarX,
                y: internalY + 60,
                rotation: 0,
                label: 'PAT',
        ...
        if (fedCircuits.length > 1) {
            ...
            fedCircuits.forEach((element: any, index: number) => {
                ...
                // 🆕 CONECTOR PAT (Línea punteada a la bornera)
                if (panel.grounding?.hasPAT) {
                    pipes.push({
                        id: generateSymbolId(`pat-line-${element.id}`),
                        type: 'line',
                        points: [patBusbarX, internalY + 50, elementX, internalY + 50],
                        color: '#475569',
                        strokeWidth: 1,
                        dash: [4, 4],
                        layer: 'layer-0'
                ...
                // 🆕 LÍNEA CT (Circuito Terminal) - Fleming saliendo de la protección
                // Se coloca solo si es un circuito terminal (no si alimenta a otro tablero)
                const isTerminal = !element.feedsTS; // Mejorar detección si es necesario
                if (isTerminal) {
                    const ctArrowY = internalY + 60;
            ...
            internalY += PROTECTION_HEIGHT + 40; // Espacio para protecciones + símbolos de línea
            ...
            internalY += PROTECTION_HEIGHT + 40;
        }
    }
